...
\begin{thm}[\textsc{Baras--Goldstein} \textbf{1984}]
...
\begin{thm}[\textsc{Cabr\'e--Martel} \textbf{1999}]
...
\item Falls $\lambda_1(\Delta + V)=-\infty$, dann folgt f\"ur $0\le u_0 \in L^2(\mathbb R^N)\setminus\{0\}$, dann gibt es keine positive L\"osung von \eqref{heat}.
\end{enumerate}
\end{thm}
\begin{proof}
%\begin{enumerate}[(i)]
...
\end{equation}
Daher folgt die lokal gleichm\"aßige Beschr\"anktheit der zugehörigen Halbgruppen mit
\begin{equation}
\|S_n(t)\| \le e^{-\lambda_1(\Delta + V)}
\end{equation}
Nach dem Trotter-Neveu-Kato Theorem (vgl. Appendix) folgt die Existenz einer stark stetigen Halbgruppe $S(t)$, sodass $S_n(t) \to S(t), t\ge 0$ im staken Sinne konvergiert.  Dann folgt im Grenzwert für $u(t) = S(t) u_0\in C([0,T],\mathbb L^2(\mathbb R^N))$, dass dies eine schwache Lösung zu \eqref{perturbed} ist.  

(ii) Angenommen es gäbe eine postive schwache Lösung zu \eqref{perturbed} mit Anfangswert $u_0\ge 0$.  Sei $u_n$ die eindeutige postive Lösung zu \eqref{cutoff2}. Dann gilt das Maximumsprinzip (vgl. Appendix)
\begin{equation}
0<u_n \le u.
\end{equation}
Mit monotoner Konvergenz folgt, die Existenz eines punktweisen Grenzwerts $u_n(t)\to \tilde u(t), t\ge 0$, die sogenannte milde Lösung des Problems.  
...
\end{equation}
Integrieren wir für $t\in (1,\infty)$, so folgt
\begin{equation}
(t-1) \int_{\mathbb R^N} V_n \phi^2\, \mathrm dx \le \int_{\mathbb R^N} \log \left ( \right )
\end{equation}
%\end{enumerate}
\end{proof}
\appendix
